...
void print_hex(const char *data, size_t length) {
    for (size_t i = 0; i < length; ++i) {
        gsm_printf("%02X ", (unsigned char)data[i]);
        if ((i + 1) % 16 == 0) {
            gsm_printf("\r\n");
        }
    }
...
void print_string(const char *data, size_t length) {
    for (size_t i = 0; i < length; ++i) {
        if (data[i] == '\0') break;
        gsm_printf("%c", data[i]);
    }
...
//  gsm_printf("[GSM] msg_send() begin\r\n");
  ...
  {
    gsm_printf("[GSM]7 msg_send() failed!\r\n");
    return false;
  }
  if (gsm_lock(10000) == false)
  {
    gsm_printf("[GSM]6 msg_send() failed!\r\n");
    return false;
  }
  ...
  if (gsm.msg.textMode == 1)
  {
    ...
    if (gsm_command(str, 5000 , NULL, 0, 2, "\r\r\n> ", "\r\nERROR\r\n") != 1)
    {
      ...
      gsm_printf("[GSM]5 msg_send() failed!\r\n");
      gsm_unlock();
      return false;
    }
    ...
    {
      gsm_printf("[GSM]4 msg_send() failed!\r\n");
      gsm_unlock();
      return false;
    }
  ...
  {		
    ...
    if(messageLen > 0)
    {
      ...
      if (gsm_command(str, 5000 , NULL, 0, 2, "\r\r\n> ", "\r\nERROR\r\n") != 1)
      {
        sprintf(str, "%c", 27);
        gsm_command(str, 1000, NULL, 0, 0);
        gsm_printf("[GSM]3 msg_send() failed!\r\n");
        gsm_unlock();
        return false;
      }
      ...
      {
        gsm_printf("[GSM]2 msg_send() failed!\r\n");
        gsm_unlock();
        return false;
      }
//      gsm_printf("[GSM] msg_send() done\r\n");
    ...
    gsm_printf("[GSM]1 msg_send() failed!\r\n");
    gsm_unlock();
    return false;
  }
  // --- pdu mode
...
//        gsm_printf("[GSM] msg_read(%d) buffer (hex):\r\n", index);
//        print_hex((char*)gsm.buffer, sizeof(gsm.buffer));
//        gsm_printf("[GSM] msg_read(%d) buffer (string):\r\n", index);
//        print_string((char*)gsm.buffer, sizeof(gsm.buffer));
//
//        int parsed = sscanf((char*)gsm.buffer, "\r\n+CMGR: \"%[^\"]\",\"%[^\"]\",\"\",\"%hd/%hd/%hd,%hd:%hd:%hd%*d\"",
//        					gsm.msg.status,gsm.msg.number, &d[0], &d[1], &d[2], &d[3], &d[4], &d[5]);
//        if (parsed != 8)
//        {
//            gsm_printf("[GSM] msg_read(%d) failed to parse response! Parsed: %d\r\n", index, parsed);
//            gsm_unlock();
//            return false;
//        }
        ...
        while (s != NULL)
        {
            ...
            if (cnt == 6)
            {
                ...
                if (end != NULL)
                {
                    ...
                    gsm_printf("[GSM] msg_read(%d) done, message: %s\r\n", index, (char*)&gsm.buffer[0]);
                    gsm_unlock();
     //->               //gsm_callback_newMsg(gsm.msg.number, gsm.msg.time, (char*)&gsm.buffer[0]);
                    return true;
                } else
                {
                    gsm_printf("[GSM] msg_read(%d) failed!\r\n", index);
    ...
    else if (gsm.msg.textMode == 0)
    {
        ...
        char *end = strstr((const char*)gsm.buffer, "\r\n\r\nOK\r\n");
        char *s = strtok((char*)gsm.buffer, "\r\n");
			s = strtok(NULL, "\r\n");
			strncpy((char*)&gsm.buffer[0], s, end - s);
			memset(&gsm.buffer[end - s], 0, sizeof(gsm.buffer) - (end - s));
